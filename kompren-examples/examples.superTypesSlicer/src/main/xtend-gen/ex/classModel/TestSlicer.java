...
public class TestSlicer {
  ...
  public void testSlicer() {
    ...
    ClassModel _createClassModel = TestSlicer.createClassModel(requiredClazzs);
    EList<Clazz> _classes = _createClassModel.getClasses();
    Clazz _head = IterableExtensions.<Clazz>head(_classes);
    requiredClazzs.add(_head);
    superTypesSlicer _superTypesSlicer = new superTypesSlicer(requiredClazzs);
    final superTypesSlicer slicer = _superTypesSlicer;
    slicer.launch();
    String _string = slicer.buf.toString();
    Assert.assertEquals("Slicing starting c3 c4 c5 c1 c2 c4 c5 c3 c4 c2 c3 c1 c2 Slicing ended", _string);
  }
